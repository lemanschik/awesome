...
module.exports = client.concat(server).concat([
  {
    external: ['@emotion/core', 'react', 'react-dom'],
    input: ["browse","main"].map(entryName=>`modules/client/${entryName}.js`),
    output: {
      format: 'esm',
      dir: 'public/_packages',
      entryFileNames: '[name]-[hash].js',
      globals: {
        react: 'React',
        'react-dom': 'ReactDOM',
  ...
  //    compiler()
    ...
    external: builtinModules.concat(dependencies),
    input: ['modules/server.js'],
    output: { file: 'public/_packages/server.js', format: 'esm' },
    moduleContext: {
      'node_modules/react-icons/lib/esm/iconBase.js': 'global'
    },
    plugins: [
